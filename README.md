# **Code Café**

A Smart Coffee Ordering System

![Java](https://img.shields.io/badge/Java-GUI-orange)
![Storage](https://img.shields.io/badge/Storage-Java%20Arrays%20%7C%20SQL-blue)
![Status](https://img.shields.io/badge/Status-Active-brightgreen)

---

## **📖 Table of Contents**

- [About the Project](#about-the-project)
- [Features](#features)
- [How It Works](#how-it-works)
- [Members](#members)

---

## **📌 About the Project**

**Code Café** is a Java GUI-based café ordering system designed to help small cafés or coffee shops manage customer orders efficiently.

The system allows users to select items from a menu that includes coffee, non-coffee drinks, and snacks, edit their selections if a mistake was made, and generate receipts. Orders are displayed in a clear and user-friendly interface and stored using Java arrays or SQL.

This project reduces manual errors, speeds up order processing, and provides a simple record of transactions.

---

## **🚀 Features**

- **Menu Selection**  
  Users can browse a categorized menu including coffee, non-coffee drinks, and snacks, and select items to order.

- **Order Summary & Receipt**  
  Displays the total price of the order and prints a simple receipt for the customer. Receipts can be saved as a file (text or PDF) for download if a printer is unavailable.

- **Order Management & Edit Orders**  
  Keeps track of current and past orders in a structured way for easy reference. Users can also edit or remove items from the order if a mistake was made before finalizing.

- **Search & Filter Menu**  
  Quickly find drinks by name or category to make ordering faster.

- **Simple Reporting**  
  Shows total orders for the day and popular menu items.

---

## **⚙️ How It Works**

1. **Menu Selection**  
   Users browse the categorized menu and select items to add to their order.

2. **Order Processing**  
   Selected items are displayed in the order summary with the calculated total price.

3. **Edit Orders**  
   Items can be removed or modified before confirming the order.

4. **Receipt Generation**  
   A receipt is generated and can be printed or saved as a text or PDF file.

5. **Order Storage**  
   Orders are stored using Java arrays or SQL for record keeping and reporting.

---

## **👨‍💻 Members**

- JUNIO, IAN CHRISTOPHER L.  
- SARMIENTO, BRYAN JOSEF E.  
- ORTE, RAINAN S.  
- FORONDA, MARC RENJO M.  
- BIACAN, TIFFANY QUENN D.
